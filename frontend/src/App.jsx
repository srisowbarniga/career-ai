...
function App() {
  ...
  const navBtn = (label, pg) => (
    <button onClick={() => setPage(pg)} style={{ background: page === pg ? "white" : "transparent", color: page === pg ? "#6366f1" : "white", border: "1px solid white", padding: "8px 16px", borderRadius: "20px", cursor: "pointer", fontWeight: "bold" }}>{label}</button>
  );
        ...
        <div style={{ maxWidth: "700px", margin: "40px auto", padding: "20px" }}>
          ...
          {!resume ? (
            ...
            <div style={{ background: "white", padding: "40px", borderRadius: "20px", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
              <div style={{ borderBottom: "3px solid #6366f1", paddingBottom: "20px", marginBottom: "20px" }}>
                <h1 style={{ color: "#6366f1", margin: 0 }}>{form.name}</h1>
                <p style={{ color: "#666", margin: "8px 0" }}>{user?.email} | Grade: {form.grade}</p>
              </div>
              <div style={{ marginBottom: "20px" }}>
                <h3 style={{ color: "#6366f1", borderBottom: "1px solid #eee", paddingBottom: "8px" }}>🎯 Objective</h3>
                <p style={{ color: "#555" }}>{resume.objective}</p>
              </div>
              <div style={{ marginBottom: "20px" }}>
                <h3 style={{ color: "#6366f1", borderBottom: "1px solid #eee", paddingBottom: "8px" }}>🎓 Education</h3>
                <p style={{ color: "#555" }}>{resume.education}</p>
              </div>
              <div style={{ marginBottom: "20px" }}>
                <h3 style={{ color: "#6366f1", borderBottom: "1px solid #eee", paddingBottom: "8px" }}>💡 Skills</h3>
                <p style={{ color: "#555" }}>{Array.isArray(resume.skills) ? resume.skills.join(", ") : resume.skills}</p>
              </div>
              <div style={{ marginBottom: "20px" }}>
                <h3 style={{ color: "#6366f1", borderBottom: "1px solid #eee", paddingBottom: "8px" }}>❤️ Interests</h3>
                <p style={{ color: "#555" }}>{Array.isArray(resume.interests) ? resume.interests.join(", ") : resume.interests}</p>
              </div>
              <div style={{ marginBottom: "20px" }}>
                <h3 style={{ color: "#6366f1", borderBottom: "1px solid #eee", paddingBottom: "8px" }}>🚀 Projects</h3>
                <p style={{ color: "#555" }}>{Array.isArray(resume.projects) ? resume.projects.join(", ") : resume.projects}</p>
              </div>
              <div style={{ display: "flex", gap: "12px", marginTop: "20px" }}>
                <button onClick={() => window.print()} style={{ background: "#6366f1", color: "white", padding: "12px 24px", border: "none", borderRadius: "10px", fontSize: "16px", cursor: "pointer" }}>🖨️ Print / Save as PDF</button>
                <button onClick={() => setResume(null)} style={{ background: "#10b981", color: "white", padding: "12px 24px", border: "none", borderRadius: "10px", fontSize: "16px", cursor: "pointer" }}>🔄 Regenerate</button>
              </div>
            </div>
          )}
        </div>
